...
```bash
npx fancy-ui@latest init        # configure the project (writes fancy.json)
npx fancy-ui@latest add card    # vendor a component + its deps
npx fancy-ui@latest list        # browse everything available
```
...
```bash
npx fancy-ui init
npx fancy-ui init --yes          # CI-friendly, all defaults
npx fancy-ui init --force        # reconfigure an existing project
```
...
npx fancy-ui add card --overwrite
npx fancy-ui add card --no-install
```
